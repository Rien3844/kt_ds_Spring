 ...
 *  - 애플리케이션의 버전이 다르다. => 0.0.1, 1.0.2 같이 다른 버전을 사용자가 동시에 사용하는 경우
 *  로 알맞는 처리를 위해 인터페이스를 제공한다.
 *   - 상황에 맞추어 클래스를 생성해서 사용자에게 제공.
 *   
 * 서비스의 목적 ==> 트랜잭션(잘 되면 커밋, 한 개라도 실패하면 롤백) 처리를 위해 쓰임.( 이체 ==> 출금 ==> 입금)
...
public interface BoardService {
	...
	BoardVO findBoardByArticleId(String articleId, ReadType view);
